...
export async function ingestConnectorMessage(channel:{id:string;workspaceId:string;employeeId:string|null;type:string},incoming:ConnectorMessage){
  ...
  // Messages are ordered by server receipt time: provider timestamps have second precision
  // and delayed webhook retries would otherwise sort a new question before earlier AI replies.
  // Provider message ids are only unique per bot/chat (Telegram numbers them 1, 2, 3…
  // in every chat), so the dedupe key must include the channel and the chat.
  const messageExternalId=`${channel.type}:${channel.id}:${incoming.externalConversationId}:${incoming.externalMessageId}`;
  try{await db.message.create({data:{conversationId:conversation.id,direction:"INBOUND",content:incoming.text,externalId:messageExternalId}})}catch(error){if(isUniqueConstraintError(error))return{duplicate:true,conversationId:conversation.id};throw error}
  ...
  if(channel.type!=="WEBSITE")await enqueueJob(channel.workspaceId,"OUTBOUND_CHANNEL_MESSAGE",{channelId:channel.id,recipientId:incoming.senderId,text:result.message.content,messageId:result.message.id,...(incoming.subject?{subject:incoming.subject}:{})});
